...
const GetUserStats = (termProp) => {
  ...
  const getObscurities = async () => {
    ...
    let finalScore = (
      10 -
      ((await getPopArtist()) + (await getPopTrack())) / 1000
    ).toFixed(2); // subtract from 10; higher obscurity = higher score
  ...
  const getPopArtist = async () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      if (indivData.popularity > currentHighest) {
        //most popular artist
  ...
  const getPopTrack = async () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      if (indivData.popularity > currentHighest) {
        //update the image url of most popular song if more popular than current highest
        currentHighest = indivData.popularity;
