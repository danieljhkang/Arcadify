...
const Home = () => {
  ...
  return (
    <>
      {!isLoggedIn && (
        <h1 className="homeText">Enter your favorite music arcade.</h1>
      )}
      {!isLoggedIn && (
      ...
      {isLoggedIn && (
        <div id="selectorArea1">
          <button className="arrow" onClick={handleSetSubjectLeft}>
            <img className="arrowImg" src={leftArrow} alt="leftArrow"></img>
          </button>
          <p id="selectionText">{subjectText}</p>
          <button className="arrow" onClick={handleSetSubjectRight}>
            <img className="arrowImg" src={rightArrow} alt="rightArrow"></img>
          </button>
        </div>
      )}
      {isLoggedIn && (
        <div id="selectorArea2">
          <button className="arrow" onClick={handleSetTermLeft}>
            <img className="arrowImg" src={leftArrow} alt="leftArrow"></img>
          </button>
          <p id="selectionText">{termText}</p>
          <button className="arrow" onClick={handleSetTermRight}>
            <img className="arrowImg" src={rightArrow} alt="rightArrow"></img>
          </button>
        </div>
      )}
      <div id="captureArea">
        ...
        {isLoggedIn &&
          (currentSubject === "top_tracks" ||
            currentSubject === "top_artists" ||
            currentSubject === "top_genres") && (
            <>
        ...
        {isLoggedIn && currentSubject === "top_tracks" && (
          <GetTopSongs termProp={{ curTerm: { currentTerm } }} />
        )}
        {isLoggedIn && currentSubject === "top_artists" && (
          <GetTopArtists termProp={{ curTerm: { currentTerm } }} />
        )}
        {isLoggedIn && currentSubject === "top_genres" && (
          <GetTopGenres termProp={{ curTerm: { currentTerm } }} />
        )}
        {isLoggedIn && currentSubject === "user_stats" && (
          <GetUserStats termProp={{ curTerm: { currentTerm } }} />
        )}
        {isLoggedIn && (
          <img
            className="machineTemplate"
            src={machineImage}
            alt="arcadeMachine"
          />
        )}
      ...
      <br />
      {isLoggedIn && (
        <button className="spotifyButton" onClick={handleDownload}>
          Download Image
        </button>
      )}
      {isLoggedIn && (
        <button
          ...
          role="link"
          onClick={() => handleLogout("https://accounts.spotify.com/en/logout")}
        >
          Log Out
        </button>
      )}
      ...
      {isLoggedIn && !isExtended && (
        <div>
          ...
          <br />
          <button className="arrow" onClick={handleExtend}>
            <img className="arrowImg" src={downArrow} alt="downArrow"></img>
          </button>
        </div>
      )}
      {isLoggedIn && isExtended && currentSubject === "top_tracks" && (
        <div>
          <GetTrackExtension
            termProp={{ curTerm: { currentTerm }, termText: { termText } }}
          />
          <br />
          <button className="arrow" onClick={handleExtend}>
            <img className="arrowImg" src={upArrow} alt="upArrow"></img>
          </button>
          <br />
